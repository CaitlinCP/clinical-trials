...
def make_trials_api_call(
    ...
    payload = {
        "fields": fields,
        "query.intr": "AREA[InterventionType]DRUG",
        "pageToken": pageToken,
        "pageSize": limit_per_call,
        "postFilter.advanced": "AREA[IsFDARegulatedDrug]true AND AREA[Phase](PHASE3 OR PHASE4)",
        "query.locn": "AREA[LocationCountry]United States",
    }

    r = requests.get("https://www.clinicaltrials.gov/api/v2/studies", params=payload)
...
def pull_trials_data(limit_per_call=1000, limit_total=float("inf"), fields=API_FIELDS):
    """
    ...
    response = make_trials_api_call(limit_per_call=limit_per_call, fields=fields).json()
    ...
    while next_page_token and count_results + limit_per_call <= limit_total:
        ...
        try:
            ...
            response = make_trials_api_call(
                limit_per_call=limit_per_call, fields=fields, pageToken=next_page_token
            ).json()
